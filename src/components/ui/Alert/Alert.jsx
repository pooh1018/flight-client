...
const Alert = ({
  ...
  // 构建类名
  const baseClass = 'custom-alert';
  const classes = [
    baseClass,
    `${baseClass}--${type}`,
    banner ? `${baseClass}--banner` : '',
    showIcon ? `${baseClass}--with-icon` : '',
    description ? `${baseClass}--with-description` : '',
    className
  ].filter(Boolean).join(' ');
  ...
  const renderIcon = () => {
    ...
    return <span className={`${baseClass}__icon`}>{iconNode}</span>;
  };
  ...
  const renderCloseIcon = () => {
    ...
    const closeIcon = closeText || (
      <span className={`${baseClass}__close-icon`}>
        <svg viewBox="64 64 896 896" width="1em" height="1em" fill="currentColor">
